...
SHOT = sys.argv[1] if len(sys.argv) > 1 else str(ROOT / "docs" / "server.jpg")
shot_html = f'<img src="{SHOT}" style="width:12.5cm"/>' if Path(SHOT).exists() else ""

HTML = f"""
<html><head><style>
  @page {{ size: letter; margin: 1.0cm 1.4cm; }}
  body {{ font-family: Helvetica, Arial, sans-serif; font-size: 9.5pt; color: #1f2328; line-height: 1.25; }}
  h1 {{ color: #5865F2; font-size: 19pt; margin: 0 0 1pt 0; }}
  .sub {{ color: #57606a; margin: 0 0 8pt 0; }}
  h2 {{ color: #5865F2; font-size: 11.5pt; margin: 7pt 0 3pt 0; }}
  ...
  .cmd {{ background: #f6f8fa; border: 1px solid #d0d7de; padding: 4pt 8pt; margin: 3pt 0; }}
  .cap {{ font-size: 9pt; color: #57606a; margin: 2pt 0 6pt 0; }}
  .gl td {{ font-size: 9pt; padding: 0 6pt 0 0; vertical-align: top; }}
...
<div class="step"><span class="n">4</span> Open the forum for your major (data-analytics, swe, cybersecurity, quant, finance-fintech, project-management, digital-marketing) and read the <b>Start here</b> post at the top.</div>
...
<tr><td><b>Pull request (PR)</b></td><td>you sending your change back to the project. A merged PR = real experience.</td><td><b>Green / yellow / red dot</b></td><td>starter / intermediate / advanced, by repo size and stars.</td></tr>
</table>
